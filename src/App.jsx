...
export default function App() {
  ...
  const setLevel = (catId, topic, level) => {
    setState(prev => ({
      ...prev,
      progress: { ...prev.progress, [catId]: { ...prev.progress[catId], [topic]: level } },
  ...
  const setNote = (catId, topic, note) => {
    setState(prev => ({
      ...prev,
      notes: { ...prev.notes, [catId]: { ...prev.notes[catId], [topic]: note } }
    }));
  };
  ...
  const deleteTopic = (catId, topic) => {
    ...
    setState(prev => {
      const newProgress = { ...prev.progress[catId] };
      delete newProgress[topic];
      const newNotes = { ...prev.notes[catId] };
      delete newNotes[topic];
      return {
        ...prev,
        customTopics: {
          ...prev.customTopics,
          [catId]: (prev.customTopics[catId] || []).filter(t => t !== topic)
        },
        progress: { ...prev.progress, [catId]: newProgress },
        notes: { ...prev.notes, [catId]: newNotes }
      };
    });
  };
  ...
  const styles = {
    app: { background: "#080c14", minHeight: "100vh", fontFamily: "'Courier New', monospace", color: "#e2e8f0" },
    header: {
      borderBottom: "1px solid #1e293b", padding: "20px 32px",
      display: "flex", alignItems: "center", justifyContent: "space-between",
      background: "rgba(0,255,200,0.03)",
    },
    logo: { fontSize: 22, fontWeight: 700, color: "#00ffc8", letterSpacing: 2, textTransform: "uppercase" },
    subtitle: { fontSize: 11, color: "#64748b", letterSpacing: 4, marginTop: 2 },
    nav: { display: "flex", gap: 4, padding: "12px 32px", borderBottom: "1px solid #1e293b", background: "#0a0f1a" },
    navBtn: (active) => ({
      padding: "6px 18px", border: "1px solid", borderColor: active ? "#00ffc8" : "#1e293b",
      borderRadius: 3, background: active ? "rgba(0,255,200,0.1)" : "transparent",
      color: active ? "#00ffc8" : "#64748b", cursor: "pointer", fontSize: 12, letterSpacing: 1,
    }),
    main: { padding: "28px 32px", maxWidth: 1100, margin: "0 auto" },
  };

  const projStatuses = ["Planlıyor", "Devam Ediyor", "Tamamlandı", "Duraklatıldı"];
  const projStatusColors = { "Planlıyor": "#64748b", "Devam Ediyor": "#3b82f6", "Tamamlandı": "#10b981", "Duraklatıldı": "#f59e0b" };

  // ---- DASHBOARD ----
  const Dashboard = () => (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20, marginBottom: 32 }}>
        <div style={{ background: "#0f1520", border: "1px solid #1e293b", borderRadius: 8, padding: 24 }}>
        ...
        {catData.map(cat => {
          ...
          return (
            <div
              ...
              style={{
                background: "#0f1520", border: `1px solid ${cat.pct > 60 ? cat.color + "44" : "#1e293b"}`,
            ...
            >
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {counts.map((c, i) => c > 0 && (
                  <span key={i} style={{ fontSize: 10, padding: "2px 6px", borderRadius: 3, background: LEVEL_COLORS[i] + "33", color: LEVEL_COLORS[i] }}>{c}</span>
                ))}
              </div>
            </div>
          );
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            ...
            <button
              ...
              style={{ padding: "8px 16px", background: "transparent", border: `1px solid ${cat.color}66`, borderRadius: 4, color: cat.color, cursor: "pointer", fontSize: 12 }}
            >+ KONU EKLE</button>
          </div>
          {addingTopic && (
            <div style={{ background: "#0f1520", border: "1px solid #1e293b", borderRadius: 8, padding: 16, marginBottom: 16, display: "flex", gap: 10 }}>
              <input
                value={newTopicText} onChange={e => setNewTopicText(e.target.value)}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {topics.map(topic => {
              ...
              return (
                <div key={topic} style={{ background: "#0f1520", border: `1px solid ${level > 0 ? LEVEL_COLORS[level] + "33" : "#1e293b"}`, borderRadius: 8, padding: "14px 18px" }}>
                      ...
                      {LEVELS.map((lbl, i) => (
                        <button key={i} onClick={() => setLevel(activeCategory, topic, i)} title={lbl}
                          style={{
                            ...
                            color: level === i ? LEVEL_COLORS[i] : "#334155", fontSize: 10, fontWeight: 700,
                          }}
                        >{i}</button>
                      ))}
                    </div>
                    <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 12, background: LEVEL_COLORS[level] + "22", color: LEVEL_COLORS[level], minWidth: 80, textAlign: "center" }}>
                      {LEVELS[level]}
                    </span>
                    <button onClick={() => setEditingNote(isEditing ? null : `${activeCategory}-${topic}`)}
                      style={{ background: "transparent", border: "none", color: "#334155", cursor: "pointer", fontSize: 16, padding: "0 4px" }} title="Not ekle">📝</button>
                    <button onClick={() => deleteTopic(activeCategory, topic)}
                      style={{ background: "transparent", border: "none", color: "#334155", cursor: "pointer", fontSize: 14, padding: "0 4px" }} title="Sil">✕</button>
                  </div>
                  {isEditing && (
                    <textarea value={note} onChange={e => setNote(activeCategory, topic, e.target.value)}
                      placeholder="Not ekle..." rows={3}
                      style={{ marginTop: 10, width: "100%", background: "#1e293b", border: "1px solid #334155", borderRadius: 4, padding: "8px 12px", color: "#e2e8f0", fontSize: 12, resize: "vertical", outline: "none", boxSizing: "border-box" }}
                      autoFocus
                    />
                  )}
                  {!isEditing && note && (
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          ...
          <button onClick={() => setAddingProject(true)}
            style={{ padding: "8px 18px", background: "transparent", border: "1px solid #06b6d466", borderRadius: 4, color: "#06b6d4", cursor: "pointer", fontSize: 12 }}>
            + YENİ PROJE
          </button>
        </div>
        {addingProject && (
          <div style={{ background: "#0f1520", border: "1px solid #1e293b", borderRadius: 10, padding: 20, marginBottom: 20 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <input defaultValue="" onChange={e => { newProject.name = e.target.value; }} placeholder="Proje adı *"
                style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 4, padding: "10px 14px", color: "#e2e8f0", fontSize: 13, outline: "none" }} />
              <input defaultValue="" onChange={e => { newProject.tech = e.target.value; }} placeholder="Teknolojiler (React, Node, ...)"
                style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 4, padding: "10px 14px", color: "#e2e8f0", fontSize: 13, outline: "none" }} />
            </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button
                onClick={() => {
                  ...
                  setState(prev => ({
                    ...
                    projects: [...(prev.projects || []), {
                      name: newProject.name || "", desc: newProject.desc || "",
                      tech: newProject.tech || "", status: newProject.status || "Planlıyor",
                ...
                style={{ padding: "8px 20px", background: "#06b6d4", border: "none", borderRadius: 4, color: "#080c14", cursor: "pointer", fontWeight: 700, fontSize: 12 }}>
                Kaydet
              </button>
              <button onClick={() => setAddingProject(false)}
                style={{ padding: "8px 16px", background: "transparent", border: "1px solid #334155", borderRadius: 4, color: "#64748b", cursor: "pointer", fontSize: 12 }}>
                İptal
              </button>
            </div>
          </div>
            ...
            <div key={p.id} style={{ background: "#0f1520", border: "1px solid #1e293b", borderRadius: 10, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                ...
                <button onClick={() => { if (window.confirm(`"${p.name}" projesini silmek istiyor musun?`)) deleteProject(p.id); }}
                  style={{ background: "transparent", border: "none", color: "#334155", cursor: "pointer", fontSize: 14, padding: 0 }}>✕</button>
              </div>
              {p.desc && <div style={{ fontSize: 12, color: "#64748b", marginBottom: 12, lineHeight: 1.5 }}>{p.desc}</div>}
              {p.tech && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                  {p.tech.split(",").map((t, i) => (
                    <span key={i} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 3, background: "#1e293b", color: "#94a3b8" }}>{t.trim()}</span>
                  ))}
                </div>
              )}
              <select value={p.status} onChange={e => updateProject(p.id, "status", e.target.value)}
                style={{ background: projStatusColors[p.status] + "22", border: `1px solid ${projStatusColors[p.status]}44`, borderRadius: 4, padding: "4px 10px", color: projStatusColors[p.status], fontSize: 11, cursor: "pointer" }}>
                {projStatuses.map(s => <option key={s} style={{ background: "#0f1520", color: "#e2e8f0" }}>{s}</option>)}
              </select>
            </div>
          ))}
          {projects.length === 0 && (
    ...
    <div style={styles.app}>
      <div style={styles.header}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 12, color: "#10b981" }}>{saveStatus || `${CATEGORIES.length} kategori`}</div>
        </div>
      </div>
